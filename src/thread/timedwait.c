...
hidden int __timedwait_cp(volatile int *fut, int val, int priv, const struct timespec *absto, int clockid)
{
    int rv;
...
hidden int __timedwait(volatile int *fut, int val, int priv, const struct timespec *absto, int clockid)
{
    int cs;
    pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &cs);
    int rv = __timedwait_cp(fut, val, priv, absto, clockid);
