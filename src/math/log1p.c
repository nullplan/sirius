...
double log1p(double x)
{
    ...
    if (hx < 0x3FDA827A) {			/* x < 0.41422  */
        ...
        if(ax < 0x3e200000) {			/* |x| < 2**-29 */
            FORCE_EVAL(two54 + x);		/* raise inexact */
    ...
    if(k != 0) {
        if (hx < 0x43400000) {
            u  = 1.0 + x; 
            hu = __double_bits(u) >> 32;	/* high word of u */
            k  = (hu >> 20) - 1023;
        ...
        hu &= 0x000fffff;
        if (hu < 0x6a09e) {
            u = __double_from_bits((__double_bits(u) << 12 >> 12) | 0x3ffULL << 52);
        } else {
            k += 1; 
            u = __double_from_bits((__double_bits(u) << 12 >> 12) | 0x3feULL << 52);
            hu = (0x00100000 - hu)>>2;
        }
        f = u - 1.0;
    }
