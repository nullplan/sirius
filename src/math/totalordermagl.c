...
int totalordermagl(const long double *pa, const long double *pb)
{
    #if LDBL_MANT_DIG==53
    return totalordermag((const double *)pa, (const double *)pb);
    #elif LDBL_MANT_DIG==64
    union ldshape lda = {*pa}, ldb = {*pb};
    uint16_t ixa = lda.i.se << 1;
    uint16_t ixb = ldb.i.se << 1;
